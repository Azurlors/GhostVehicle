...
class Segment:
    ...
    def add_vehicle(self, veh, ghost=False, attacker=None):
        if ghost:
            print('adding ghost vehicle')
            # insert vehicle before attacker
            find = False
            for i in range(len(self.vehicles)):
                if self.vehicles[i] == attacker.id:
                    find = True
                    self.vehicles.insert(i, veh.id)
                    break
            if not find:
